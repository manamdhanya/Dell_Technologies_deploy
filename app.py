...
lms_summary = filtered_lms[['SessionDuration', 'PagesViewed']].mean()
st.dataframe(lms_summary.to_frame().transpose())
...
st.subheader("⚠️ Data Poisoning Simulation (Optional)")
noise = st.slider("Add Noise Level", 0.0, 1.0, 0.0)

poisoned_absence = absence_rate + (noise * 0.5)
poisoned_session = session_duration * (1 - noise)
poisoned_pages = pages_viewed * (1 - noise)

if noise > 0:
    st.warning("⚠️ Poissoned (Manipulated) Input Data:")
    st.write({
        "AbsenceRate": poisoned_absence,
        "SessionDuration": poisoned_session,
        "PagesViewed": poisoned_pages
    })

if st.button("Predict (Clean Data)"):
    prediction = model.predict([[absence_rate, session_duration, pages_viewed]])
    result = "Engaged" if prediction[0] == 1 else "At Risk"
    st.success(f"Clean Data Prediction: {result}")

if st.button("Predict (Poisoned Data)"):
    prediction = model.predict([[poisoned_absence, poisoned_session, poisoned_pages]])
    result = "Engaged" if prediction[0] == 1 else "At Risk"
    st.error(f"Poisoned Data Prediction: {result}")
